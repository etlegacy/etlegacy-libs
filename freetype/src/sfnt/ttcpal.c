...
#define FT_COMPONENT  ttcpal
                     ...
                     FT_Stream  stream )
  {
    ...
    if ( colors_offset >= table_size )
      goto InvalidTable;
    if ( cpal->num_colors * (FT_ULong)COLOR_SIZE > table_size - colors_offset )
      goto InvalidTable;
    ...
    if ( cpal->version == 1 )
    {
      ...
      if ( type_offset )
      {
        if ( type_offset >= table_size )
          goto InvalidTable;
        if ( face->palette_data.num_palettes * (FT_ULong)2 >
      ...
      if ( entry_label_offset )
      {
        if ( entry_label_offset >= table_size )
          goto InvalidTable;
        if ( face->palette_data.num_palette_entries * (FT_ULong)2 >
